...
-- Dropdown NPC local drop = Instance.new("TextButton", main) drop.Size = UDim2.new(1, -20, 0, 30) drop.Position = UDim2.new(0, 10, 0, 40) drop.Text = "Pilih NPC" drop.TextColor3 = Color3.new(1,1,1) drop.Font = Enum.Font.Gotham drop.TextScaled = true drop.BackgroundColor3 = Color3.fromRGB(60,60,60)

local dropFrame = Instance.new("Frame", drop) dropFrame.Position = UDim2.new(0, 0, 1, 0) dropFrame.Size = UDim2.new(1, 0, 0, #npcList * 25) dropFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40) dropFrame.Visible = false dropFrame.ClipsDescendants = true

local selectedNPC = nil for i, npc in ipairs(npcList) do local opt = Instance.new("TextButton", dropFrame) opt.Size = UDim2.new(1, 0, 0, 25) opt.Position = UDim2.new(0, 0, 0, (i - 1) * 25) opt.BackgroundColor3 = Color3.fromRGB(70, 70, 70) opt.Text = npc opt.TextColor3 = Color3.new(1,1,1) opt.Font = Enum.Font.Gotham opt.TextScaled = true
...
drop.MouseButton1Click:Connect(function() dropFrame.Visible = not dropFrame.Visible end)

-- Tombol Teleport local teleportBtn = Instance.new("TextButton", main) teleportBtn.Size = UDim2.new(1, -20, 0, 30) teleportBtn.Position = UDim2.new(0, 10, 0, 80) teleportBtn.Text = "Teleport" teleportBtn.TextColor3 = Color3.new(1, 1, 1) teleportBtn.BackgroundColor3 = Color3.fromRGB(50, 100, 50) teleportBtn.Font = Enum.Font.Gotham teleportBtn.TextScaled = true
...
merchantBtn.MouseButton1Click:Connect(function() local foundKvjesm = false for _, obj in pairs(workspace:GetChildren()) do if obj.Name == "Kvjesm" and obj:IsA("Model") then local root = obj:FindFirstChild("HumanoidRootPart") or obj:FindFirstChildWhichIsA("BasePart") if root then hrp.CFrame = root.CFrame + Vector3.new(0, 5, 0) foundKvjesm = true break end end end
